...
export const useLibrarySaveAction = ({ type, history, item, createNotice, CloudUI }) => {
	...
	const [showLibraryDialog, SaveDialog] = Modal.useDialog({
		...
		message: <LibrarySelect ref={selectRef} type={type} item={item} CloudUI={CloudUI} />,
		buttons: [
			{
				label: __('Cancel'),
...
const LibrarySelect = forwardRef(({ type, item, CloudUI }, ref) => {
	...
	const saveAttachment = (success, error) => {
		wpRest.libraries().exportImage(item.id, library).then(success).catch(error);
	};
				...
				<select value={library} onChange={(e) => setLibrary(e.target.value)}>
					...
					{teams.map(({ id, name }, i) => (
						<optgroup key={i} label={name}>
							{libraries.team[id] &&
								libraries.team[id].map(({ id, name, permissions }, i) => {
									if (!permissions.edit_items) {
										return null;
									}
									return (
