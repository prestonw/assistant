...
const Attachments = ({ baseURL, listStyle = '', className, ...rest }) => {
				...
				getItemProps={(item, defaultProps) => {
					...
					const ThumbIcon = () => {
						...
						if (item.isTrashed) {
							return (
								<div
									className="fl-asst-list-thumb-icon"
									style={{ color: 'var(--fluid-destructive-color)' }}
								>
									<Icon.Trash />
								</div>
							);
					...
					return {
						...
						children:
							'grid' === listStyle
								? (props) => <GridItem item={item} {...props} />
		...
		<Layout.AspectBox className="fl-asst-attachment-grid-item" style={{ width: '100%' }}>
			<div
				...
				style={{
					...
					alignItems: 'center',
					opacity: item.isTrashed || item.isTrashing ? 0.5 : 1,
				}}
			>
				{('image' === type || 'pdf' === item.subtype) && thumbnail && (
					<img
						src={thumbnail}
