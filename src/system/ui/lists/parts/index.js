...
const InfoItem = ({
	...
	const renderMarks = (marks) =>
		marks.map((mark, i) => {
			return (
				<span key={i} className="fl-asst-list-item-mark">
					{mark}
				</span>
			);
		});
	...
	return (
		<Tag className={classes}>
			...
			<div className="fl-asst-list-item-default-content-row">
				{(hasThumbnail || shouldAlwaysShowThumbnail) && (
					<div className={thumbClasses}>
						...
						{color && (
							<div
								className="fl-asst-list-item-color-thumbnail"
				...
				{accessories && (
					<div className="fl-asst-list-item-accessory" onClick={stopProp}>
						{accessories}
					</div>
				)}
			</div>
			...
			{itemExtras && (
				<div className="fl-asst-list-item-extras" onClick={stopProp}>
					{itemExtras}
				</div>
			)}

			{0 < marks.length && <div className="fl-asst-list-item-marks">{renderMarks(marks)}</div>}
		</Tag>
	);
};

export const Item = ({ children, className, tag: Tag = 'li', to, ...rest }) => {
	const classes = classname(
		{
...
export const NoResultsMessage = ({ message }) => {
	return <MessageBox>{message ? message : __('No Results Found')}</MessageBox>;
};
