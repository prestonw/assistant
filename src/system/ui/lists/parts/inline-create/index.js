...
const InlineCreate = ({
	...
	return (
		<>
			<CreateItem typeLabel={typeLabel} onCreate={onCreate} type={postType} />
			{items.map((item, i) => {
				const { title } = item;
				return (
					<Layout.Box
						key={i}
						className="fl-asst-inline-item fl-asst-list-item-content-info"
						padX={false}
						padY={false}
					>
						<Layout.Row
							className="fl-asst-list-item-default-content-row"
							gap="var(--fluid-med-space)"
						>
							...
							<div className="fl-asst-list-item-subject">
								...
								<div className="fl-asst-list-item-description">{__('Creating...')}</div>
							</div>
						</Layout.Row>
					</Layout.Box>
				);
...
const CreateItem = ({ onCreate = () => {}, typeLabel, type }) => {
	...
	return (
		<Layout.Box
			className="fl-asst-inline-item fl-asst-list-inline-create-item fl-asst-list-item-content-info"
			padX={false}
			padY={false}
		>
			<Layout.Row className="fl-asst-list-item-default-content-row" gap="var(--fluid-med-space)">
				...
				<div className="fl-asst-list-item-subject">
					<Form.Input
						...
						placeholder={
							'fl_code' === type
								? sprintf('Create New %s File', typeLabel)
								: sprintf('Create New %s', typeLabel)
						}
						onKeyPress={keyPress}
						onInput={(e) => setVal(e.target.value)}
						after={
							val && (
								<Button status="primary" onClick={onClick}>
									<Icon.Return />
								</Button>
							)
						}
