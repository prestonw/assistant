			...
			getItemProps={(item, defaultProps) => {
				...
				const Accessory = () => {
					if (item.isTrashed && 'trash' !== type) {
						return (
							<Button onClick={untrashComment} tabIndex="-1">
								{__('Restore')}
							</Button>
						);
					} else if (false === item.isTrashed && 'trash' == type) {
						return (
							<Button onClick={trashComment} tabIndex="-1">
								{__('Trash')}
							</Button>
						);
					}
					if (item.isSpam && 'spam' !== type) {
						return (
							<Button onClick={unspamComment} tabIndex="-1">
								{__('Restore')}
							</Button>
						);
					}
					if (item.isunSpam && 'spam' == type) {
						return (
							<Button onClick={spamComment} tabIndex="-1">
								{__('Spam')}
							</Button>
						);
					}
				...
				const Extras = () => {
					if (item.isCloning || item.isTrashing || item.isRestoring) {
						return null;
					}
					return (
