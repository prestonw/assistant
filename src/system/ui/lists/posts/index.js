		...
		<List.WordPress
			...
			getItemProps={(item, defaultProps) => {
				...
				const restorePost = () => {
					...
					wpRest
						.posts()
						.update(item.trashedItem.id, 'untrash')
						.then(() => {
							updateItem(item.trashedItem.uuid, {
								...item.trashedItem,
								isRestoring: false,
				...
				const Accessory = () => {
					if (item.isTrashed) {
						return (
							<Button onClick={restorePost} tabIndex="-1">
								{__('Restore')}
							</Button>
						);
					}
...
const BigThumbnail = ({ item }) => {
	const Wrap = (props) => (
		<div style={{ padding: '0 var(--fluid-lg-space) var(--fluid-med-space)' }} {...props} />
	);

	if (!item.thumbnailData) {
		return (
			<Wrap>
				<Layout.AspectBox ratio="5:3"></Layout.AspectBox>
			</Wrap>
		);
	}

	const { thumbnail, sizes, alt, title, height, width } = item.thumbnailData;
