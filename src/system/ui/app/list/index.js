...
const AppList = memo(
	({ className, onSortStart = () => {}, onSortEnd = () => {}, children: Item, limit, ...rest }) => {
		...
		const filtered = appOrder
			.filter((handle) => {
				return Object.keys(apps).includes(handle) && false !== apps[handle].shouldShowInAppList;
			})
			.filter((_, i) => {
				if (limit) {
					return i + 1 <= limit;
