...
export const Attachment = ({ CloudUI }) => {
	...
	const { hasChanges, submitForm, renderForm, values } = Form.useForm({
		sections,
		onSubmit,
		defaults,
	});
	...
	const ToolbarActions = () => {
		return (
			<>
				<Button disabled={!hasChanges} isSelected={hasChanges} onClick={submitForm}>
...
const Hero = memo(({ width, sizes, height, alt, type, url, mime, thumbnail }) => {
	...
	if ('audio' == type || 'video' == type) {
		...
		mediaContent = (
			<img src={thumbnail} srcSet={srcSet} height={height} width={width} alt={alt} loading="lazy" />
		);
	}

	return (
		<Layout.AspectBox className="fl-asst-hero-image" height={height} width={width}>
			{mediaContent}
		</Layout.AspectBox>
	);
});
