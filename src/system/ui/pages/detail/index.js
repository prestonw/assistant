	...
	return (
		<Tag className={classes} {...rest}>
			{notices && 0 < notices.length && (
				<div style={{ position: 'absolute', width: '100%', zIndex: 1 }}>{renderNotices()}</div>
			)}
			...
			<div className="fluid-detail-page-toolbar">
				...
				{toolbarActions && <div className="fluid-detail-page-toolbar-area">{toolbarActions}</div>}
			</div>
			{(title || thumbnail) && (
				<div className="fluid-detail-page-hero">
					<div className="fluid-hero-group">
						{title && (
							<motion.div layout="position" className="fluid-detail-page-title-text">
								{title}
							</motion.div>
						)}
						{thumbnail && (
			...
			<div className="fluid-detail-page-content-area">{children}</div>
		</Tag>
	);
};
