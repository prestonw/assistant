...
const WidgetLayout = ({ tag: Tag = 'ul', handle, before, after, ...rest }) => {
	...
	return (
		...
		>
			...
			{0 === widgets.length && (
				...
				>
					{__('No widgets, bummer. Hit the + and add some.')}
				</li>
			)}
			...
			{0 < widgets.length &&
				widgets.map((widget, i) => {
					const type =
						'type' in widget && widget.type in types ? types[widget.type] : defaultWidgetType;
