		...
		<div className="fl-asst-comment-meta">
			{url && (
				<span className="fl-asst-comment-meta-value">
					...
					<a href={url} target="_blank" rel="noopener noreferrer" onClick={stopProp}>
						{url}
					</a>
				</span>
			)}
...
const Reply = ({ onDismiss, id, postID }) => {
	const ref = useRef();
	...
	if (replyWasPosted) {
		return (
			...
			>
				{__('Success! Your reply was posted.')}
			</Layout.Row>
		);
	}

	return (
		<motion.div style={{ padding: '20px 0 5px' }} initial={{ scale: 0 }} animate={{ scale: 1 }}>
			<textarea
				ref={ref}
				onClick={stop}
			...
			<Layout.Row style={{ padding: '5px 0' }}>
				<Button onClick={dismiss}>{__('Cancel')}</Button>
				<Button status="primary" onClick={reply} style={{ marginLeft: 'auto' }}>
					{__('Reply')}
				</Button>
			</Layout.Row>
		</motion.div>
		...
		>
			<Button to={to} style={{ display: 'flex', flexDirection: 'row', padding: 0 }}>
				...
				<div className="fl-asst-comment-cell">
					...
					<LimitContent>
						<div
							className="fl-asst-comment-content"
							dangerouslySetInnerHTML={{ __html: content }}
						/>
					</LimitContent>

					{isReplying && <Reply id={id} postID={post.id} onDismiss={() => setIsReplying(false)} />}
				</div>
			</Button>
