...
const Labels = () => {
	...
	const rows = labels.map((label) => {
		if (editingLabel && editingLabel.id === label.id) {
			return {
				edit: (
					<>
						<Text.Title style={{ marginBottom: 'var(--fluid-med-space)' }}>
							{__('Edit Label')}
						</Text.Title>
						<input
							type="text"
						...
						<div style={{ display: 'flex', flex: '1 1 auto' }}>
							...
							<Button onClick={saveLabel} style={{ marginLeft: 'auto' }} status="primary">
								{__('Save')}
							</Button>
						</div>
					</>
	...
	return (
		<div className="fl-asst-edit-labels">
			<p style={{ marginTop: 0 }}>
				{__(
					'Labels allow you to mark posts or pages for organization and collaborate with other users. Below you can add more labels and change the name of existing ones. Add labels to posts inside the Content app.',
				)}
			</p>
			...
			>
				...
				/>
				<Control.CirclePicker value={newColor} onChange={(value) => setNewColor(value)} />
