...
const Main = ({ baseURL }) => {
	...
	return (
		<div className="fl-asst-bbapp-wrap">
			<div className="fl-asst-bbapp-tabs">
				<button
			...
			<div className="fl-asst-tab-content">
				{activeTab === 'libraries' && !isCloudConnected && <CloudConnect baseURL={baseURL} />}

				{activeTab === 'libraries' && isCloudConnected && (
					<Libraries preloadedLib={libraries} preloadedTeams={teams} />
				)}

				{activeTab === 'showcase' && <CommunityApp baseURL={baseURL} showcase={1} />}
			</div>
		</div>
	);
};
