...
const PreviewButton = ({ item }) => {
	...
	return (
		<Button onClick={previewPost} disabled={previewing}>
			{__('Preview')}
		</Button>
	);
};
...
const UpdateButton = ({ item }) => {
		...
		buttons.push({
			...
			onClick: ({ closeDialog }) => {
				const message = sprintf(
					__(
						'Do you really want to update this library item and replace its content with the current %s? This cannot be undone.',
					),
					type.toLowerCase(),
				);
				...
				const doRequest = (screenshot = '') => {
					librariesApi
						.syncLibraryPost(id, item, { screenshot })
						.then((response) => {
	...
	return (
		<>
			<Button onClick={showUpdateDialog}>{__('Update')}</Button>
			<UpdateDialog />
		</>
	);
};
...
const UpdateDialogContent = () => {
	...
	if (!isSingular) {
		return __(
			"Updating is currently not available. To update this library item, navigate to the post or page you'd like to update it with and click the update button again.",
		);
	}
	...
	return (
		<>
			...
			<div>
				{sprintf(
					__(
						'Updating this library item will replace its content with the content of the current %s.',
					),
					type.toLowerCase(),
				)}
			</div>
...
const ImportButton = ({ item }) => {
	...
	const replacePost = () => {
		if (!currentPageView.isSingular) {
			alert(
				__(
					"Replacing is currently not available. Navigate to the post or page you'd like to replace and click the import button again.",
				),
			);
			setAction(null);
			return;
		...
		const message = sprintf(
			__(
				'Do you really want to replace the current %s with this library item? This cannot be undone.',
			),
			currentPageView.type.toLowerCase(),
		);
		...
		librariesApi
			.syncPost(currentPageView.id, item)
			.then((response) => {
				setImportingMedia(true);
	...
	return (
		<>
			<select value={''} onChange={(e) => setAction(e.target.value)} disabled={action}>
				<option value="">
