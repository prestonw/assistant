...
export const getActions = (item, actions) => {
	...
	const maybeShowThemeWarning = () => {
		...
		if (parent) {
			alert(
				sprintf(
					__('The themes "%s" or "%s" must be active to apply or preview these settings.'),
					parent.name,
					name,
				),
			);
	...
	const importThemeSettings = () => {
		if (maybeShowThemeWarning()) {
			...
			!confirm(
				__(
					'Do you really want to import these theme settings? This will update your site design and cannot be undone.',
				),
			)
		) {
			return;
