...
export default ({ headline = '', type = 'user', team = null }) => {
	...
	return (
		<Layout.Box className="fl-asst-libraries-grid" padX={false} padY={false}>
			{!isAddingNew && (
			...
			{isAddingNew && (
				<Layout.Toolbar>
					...
					<Button onClick={() => setIsAddingNew(false)} style={{ marginLeft: 5 }}>
						<Icon.CloseCompact />
					</Button>
				</Layout.Toolbar>
			)}
			...
			<Libraries.LibrariesList type={type} team={team} maxItems={6} />
		</Layout.Box>
	);
};
