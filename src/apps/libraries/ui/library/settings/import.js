...
export default () => {
					...
					await cloud.libraries.getItem(item.id).then(async (itemResponse) => {
						await api.importPost(itemResponse.data).then(async (postResponse) => {
							if (
								...
								postResponse.data.error_code === 'post_type_not_registered'
							) {
		...
		if (invalidItemCount < completedItemCount) {
					...
					<>
						...
						{invalidItemCount > 0 && (
							<>
								{' '}
								{__(
									'Some items were not able to be imported due to these post types not being registered on this site: ',
								)}
								<br />
								<ul>{invalidPostList}</ul>
							</>
						)}
					</>
				),
			...
			createNotice({
				...
				content: (
					<>
						{__(
							'The selected items were not able to be imported due to these post types not being registered on this site: ',
						)}
						<br />
						<ul>{invalidPostList}</ul>
					</>
			...
			<Button
				onClick={() => {
					if (
						confirm(
							__(
								'Are you sure you want to import this entire library into your site? This will import all items, including templates and media files. This action cannot be undone.',
							),
						)
					) {
						importItems();
	...
	return (
		<div style={{ gridColumn: 'span 2' }}>
			...
			<LoadingBar progress={(completedItemCount / selectedItems.length) * 100} />
			<p
				style={{
					fontStyle: 'italic',
					fontSize: '12px',
			...
			>
				{null !== currentItem && !importComplete && sprintf(__('Importing %s'), currentItem.name)}
