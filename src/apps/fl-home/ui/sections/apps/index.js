...
const GoToButton = ({ children, ...rest }) => (
	<Button appearance="transparent" {...rest}>
		{children}
		...
		>
			<Swiper>
				<Card
					headline={
						<div>
							Share <strong>Assets</strong> across all your WordPress sites
						</div>
					}
					subtext={__(
						'Get access to all your templates & assets on all your sites with Assistant Pro.',
					)}
					style={{
				...
				<Card
					headline={
						<div>
							Find <strong>Content</strong> Fast
						</div>
					}
				...
				<Card
					headline={
						<div>
							<strong>Media</strong> At Your Fingertips
						</div>
					}
					subtext={__(
						'The media app lets you quickly upload and find attachment info no matter where you are.',
					)}
					style={{
...
const Card = ({ headline, subtext, footer, ...rest }) => {
	return (
		<div className="fl-asst-swiper-item" {...rest}>
