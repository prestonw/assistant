...
const MediaSection = ({ ...rest }) => {
	...
	const HeaderActions = () => (
		<Button to="/fl-media" appearance="transparent">
			{__('Media App')}
		</Button>
	);

	return (
		<Section title={__('Media')} headerActions={<HeaderActions />} padContent={false} {...rest}>
...
const MediaGrid = ({ items = [], type = 'image', types = [], setType = () => {} }) => {
...
const HoverView = () => {
	return <div className="media-hover-display">{__('You can just drop that anywhere')}</div>;
};
